...
@section('content')
    ...
    <section class="sidebar">
      ...
      <ul class="sidebar-menu" data-widget="tree">
        ...
        <li class=""><a href="admin/qladmin/danh-sach-admin"><i class="fa fa-user-secret"></i> <span>Quản lí admin</span></a></li>
         <li class="active"><a href="admin/qladmin/danh-sach-chude"><i class="fa fa-barcode"></i> <span>Quản lí chủ đề</span></a></li>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        <li><a href="#"><i class="fa fa-dashboard"></i> Quản lí chủ đề</a></li>
          ...
          <table class="table table-hover">
            <tbody class="table-data">
              ...
              <tr>
                <td>{{$ad->MaCD}}</td>
                <td>{{$ad->TenChuDe}}</td>
                <td> <a data-toggle="modal" data-target="#modal-default-edit" id-edit="{{$ad->MaCD}}"  class="btn btn-app btn-edit-admin-modal eid-edit-{{$ad->MaCD}}" style="float: left; border-radius: 3px; position: relative; padding: 0px 0px; min-width: 20px; height: 20px; text-align: center; color: #666; border: none; background-color: #00000000; font-size: 12px;margin: 0;"><i style=" float: left;color: #ff8f00; " class="fa fa-edit"></i></a>
                 <a id-del="{{$ad->MaCD}}" data-toggle="modal" data-target="#modal-default-del" class="btn btn-app btn-del-admin-modal eid-del-{{$ad->MaCD}}" style="float: left; border-radius: 3px; position: relative; padding: 0px 0px; min-width: 20px; height: 20px; text-align: center; color: #666;border: none; background-color: #00000000; font-size: 12px;margin-left: 11px;margin: 0px 0px 0px 10px;"><i style=" float: left;color: red; " class="fa fa-ban"></i></a></td>
              </tr>
              @endforeach
            </tbody>
          </table>
  ...
  <div class="modal fade" id="modal-default-add" style="display: none;">
    ...
    <div class="form-group">
      ...
      <input name="TenChuDe" type="text" class="form-control" id="TenChuDe" placeholder="Nhập tên chủ đề">
      </div>
          </div>
          <!-- /.box-body -->
...
<!-- Modal edit -->
  <div class="modal fade" id="modal-default-edit" style="display: none;">
    ...
    <div class="form-group">
      <input  type="hidden" name="_token" value="{{csrf_token()}}">
      <input  type="hidden" name="eMaCD" value="" id="eMaCD">
      <label for="eTenChuDe">Tên chủ đề</label>
      <input name="eTenChuDe" type="text" class="form-control" id="eTenChuDe" placeholder="Nhập tên chủ đề">
      </div>
...
<script type="text/javascript">
  ...
  $(document).ready(function(){
    ...
    var a= [];
       ...
       $.ajax({
                    type: 'POST',
                    url: 'admin/qladmin/them-chude',
                    data: JSON.stringify({TenChuDe: $('#TenChuDe').val() }),
...
// code xử lí sửa
  $("#btn-edit-admin").click(function(){
    ...
    var a= [];
       ...
       $.ajax({
                    type: 'POST',
                    url: 'admin/qladmin/sua-chude',
                    data: JSON.stringify({eMaCD: $('#eMaCD').val() , eTenChuDe: $('#eTenChuDe').val(),}),
                    contentType: 'application/json; charset=utf-8',
                    dataType: 'json',
                    success: function (msg,data) {
                         ...
                         // $('.eid-edit-'+ $('#eIdAdmin').val()).parent().parent().children().eq(3).val($('#eHoTenAdmin'));
                          console.log($('.eid-edit-'+ $('#eMaCD').val()).parent().parent().children().eq(1).html($('#eTenChuDe').val()));
 ...
 $("#btn-del-admin").click(function(){
    ...
    var a= [];
       ...
       $.ajax({
                    ...
                    type: 'POST',
                    url: 'admin/qladmin/xoa-chude',
                    data: JSON.stringify({MaCD: idclickdel}),
                    contentType: 'application/json; charset=utf-8',
                    dataType: 'json',
                    success: function (msg,data) {
                          console.log(idclickdel);
