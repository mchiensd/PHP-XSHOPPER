             ...
             <li class=""><a href="admin/qladmin/danh-sach-loaihang"><i class="fa fa-tags"></i> <span>Quản lí loại hàng</span></a></li>
               <li class=""><a href="admin/qladmin/danh-sach-mausac"><i class="fa fa-industry"></i> <span>Quản lí màu sắc</span></a></li>
                 <li class=""><a href="admin/qladmin/danh-sach-nhasanxuat"><i class="fa fa-cubes"></i> <span>Quản lí nhà sản xuất</span></a></li>
                   <li class="active"><a href="admin/qladmin/danh-sach-sanpham"><i class="fa fa-cube"></i> <span>Quản lí sản phẩm</span></a></li>
        ...
        <div class="box-body table-responsive no-padding">
          <table class="table table-hover">
            <tbody class="table-data">
              ...
              @foreach($quanao as $ad)
              <tr>
                <td>{{$ad->MaQuanAo}}</td>
                <td>{{$ad->TenQuanAo}}</td>
                <td style="background: url(admin_asset/testpic/products/{{$ad->AnhBia}}); background-size: cover; width: 85px; background-repeat: no-repeat; border-radius: 10px;"></td>
                <th>{{$ad->GiaBan}}</th>
                  @foreach($kichthuoc as $cd)
                  @if($ad->MaKichThuoc == $cd->MaKichThuoc)
                   <th id-mcd-kt="{{$ad->MaKichThuoc}}">{{$cd->TenKichThuoc}}</th>        
                   @else
                   @endif
                @endforeach

                 @foreach($mausac as $cd)
                  @if($ad->MaMauSac == $cd->MaMauSac)
                   <th style="background: {{$cd->MaMauCss}};" id-mcd-ms="{{$ad->MaMauSac}}">{{$ad->MaMauSac}} <br> {{$cd->TenMauSac}} </th>        
                   @else
                   @endif
                @endforeach
                <td>{{$ad->ChatLieu}}</td>
                <td style=" width: 40%;    height: 120px; ">{{$ad->MoTa}}</td>
                <td>{{$ad->SoLuongTon}}</td>
                <td>{{$ad->MaNSX}}</td>
